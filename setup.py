...
where json_args contains:
    ...
    gpu_sm      — GPU compute capability as integer (e.g. 61 for Pascal, 86 for Ampere) or ROCm version for AMD

Example (manual test):
    python setup.py '{"python_exe":"C:/…/python.exe","ext_dir":"C:/…/hunyuan3d-2-mini","torch_flavor":"cuda","gpu_sm":86}'
"""
import json
import platform
import subprocess
...
def setup(python_exe: str, ext_dir: Path, torch_flavor: str, gpu_sm: int) -> None:
    venv = ext_dir / "venv"
    is_win = platform.system() == "Windows"
    ...
    if torch_flavor == "rocm":
            ...
            print(f"[setup] GPU SM {gpu_sm} -> PyTorch 2.9 + ROCM")
    else:
        if gpu_sm >= 70:
...
if __name__ == "__main__":
    ...
    # Positional: python setup.py <python_exe> <ext_dir> <torch_flavor> <gpu_sm>
    # JSON:       python setup.py '{"python_exe":"...","ext_dir":"...","torch_flavor":"...","gpu_sm":86}'
    if len(sys.argv) >= 5:
        setup(
            python_exe = sys.argv[1],
            ext_dir    = Path(sys.argv[2]),
            torch_flavor = sys.argv[3],
            gpu_sm     = int(sys.argv[4]),
        )
    elif len(sys.argv) == 2:
        ...
        setup(
            ...
            torch_flavor = args["torch_flavor"],
            gpu_sm     = int(args["gpu_sm"]),
        )
    else:
        print("Usage: python setup.py <python_exe> <ext_dir> <torch_flavor> <gpu_sm>")
        print('   or: python setup.py \'{"python_exe":"...","ext_dir":"...","torch_flavor":"...","gpu_sm":86}\'')
        sys.exit(1)
